...
void Aimbot::CorrectMovement(const Vector& vOldAngles, UserCmd* pCmd, float fOldForward, float fOldSidemove)
{
	float deltaView, f1, f2;
	if (vOldAngles.y < 0.f) f1 = 360.0f + vOldAngles.y;
	else  f1 = vOldAngles.y;

	if (pCmd->viewangles.y < 0.0f) f2 = 360.0f + pCmd->viewangles.y;
	else f2 = pCmd->viewangles.y;

	if (f2 < f1) deltaView = abs(f2 - f1);
	else deltaView = 360.0f - abs(f1 - f2);
    ...
    if (config.aimbot[weaponIndex].enabled && (cmd->buttons & UserCmd::IN_ATTACK || config.aimbot[weaponIndex].autoShot || config.aimbot[weaponIndex].aimlock) && activeWeapon->getInaccuracy() <= config.aimbot[weaponIndex].maxAimInaccuracy) {
        ...
        if (bestTarget && (config.aimbot[weaponIndex].ignoreSmoke
			...
			if (config.aimbot[weaponIndex].autoShot && activeWeapon->nextPrimaryAttack() <= memory.globalVars->serverTime() && !clamped && activeWeapon->getInaccuracy() <= config.aimbot[weaponIndex].maxShotInaccuracy)
				if(config.aimbot[weaponIndex].autoScope && activeWeapon->nextPrimaryAttack() <= memory.globalVars->serverTime() && activeWeapon->isSniperRifle() && !localPlayer->isScoped()) cmd->buttons |= UserCmd::IN_ATTACK2;
				else
				{
