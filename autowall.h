...
static bool TraceToExit(Vector& end, Trace* enter_trace, Vector start2, Vector dir, Trace& exit_trace)
{
	float distance = 0.0f;
	...
	while (distance <= 90.0f)
	{
		EngineTrace trace;
		distance += 4.0f;
		end = start2 + dir * distance;
		auto point_contents = trace.getPointContents(end, MASK_SHOT_HULL | CONTENTS_HITBOX);
		...
		Ray start(end, new_end);
		trace.traceRay(start, MASK_SHOT, 0, exit_trace);

		if (exit_trace.startSolid && exit_trace.surface.flags & SURF_HITBOX)
		{
			Ray start(end, start2);

			TraceFilter *filter;
			filter->skip = exit_trace.entity;

			trace.traceRay(start, 0x600400B, *filter, exit_trace);
		...
		if (!(exit_trace.fraction < 1.0 || exit_trace.allSolid || exit_trace.startSolid) || exit_trace.startSolid)
		{
			...
			{
				if (enter_trace->entity && enter_trace->entity == interfaces.entityList->getEntity(Aimbot::targetAimbot)) //need to add this in i guess
					return true;
			}
