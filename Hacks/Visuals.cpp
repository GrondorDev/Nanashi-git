...
void Visuals::thirdperson() noexcept
{
	...
	if (config.visuals.thirdperson)
		if (memory.input->isCameraInThirdPerson = (!config.visuals.thirdpersonKey || isInThirdperson) && interfaces.entityList->getEntity(interfaces.engine->getLocalPlayer())->isAlive()) 
		{
			config.GlobalIsThirdPerson = 1;
			memory.input->cameraOffset.z = static_cast<float>(config.visuals.thirdpersonDistance);
		}
		else config.GlobalIsThirdPerson = 0;
}
...
{	
	auto localPlayer = interfaces.entityList->getEntity(interfaces.engine->getLocalPlayer());
	float CurrentTime = localPlayer->tickBase() * memory.globalVars->intervalPerTick;

	for (int i = 0; i < config.DamageIndicator.size(); i++)
	{
		if (config.DamageIndicator[i].flEraseTime < CurrentTime)
		{
			config.DamageIndicator.erase(config.DamageIndicator.begin() + i);
			continue;
		}

		if (!config.DamageIndicator[i].bInitialized)
		{
			config.DamageIndicator[i].Position = config.DamageIndicator[i].Player->getBonePosition(8);
			config.DamageIndicator[i].bInitialized = true;
		}

		if (CurrentTime - config.DamageIndicator[i].flLastUpdate > 0.0001f)
		{
			config.DamageIndicator[i].Position.z -= (0.1f * (CurrentTime - config.DamageIndicator[i].flEraseTime));
			config.DamageIndicator[i].flLastUpdate = CurrentTime;
		}

		Vector ScreenPosition;

		if (worldToScreen(config.DamageIndicator[i].Position, ScreenPosition))
		{
			//drawManager->drawString(Menu->indicator_font, true, ScreenPosition.x, ScreenPosition.y, Color(255, 0, 0, 255), std::to_string(config.DamageIndicator[i].iDamage).c_str());
			interfaces.surface->setTextFont(Surface::font);
			Config::DamageIndicator_t DamageIndicator2;
			if (DamageIndicator2.rainbow)
				interfaces.surface->setTextColor(rainbowColor(memory.globalVars->realtime, DamageIndicator2.color.rainbowSpeed));
			else
				interfaces.surface->setTextColor(DamageIndicator2.color.color);


			if (wchar_t name[128]; MultiByteToWideChar(CP_UTF8, 0, std::to_string(config.DamageIndicator[i].iDamage).c_str(), -1, name, 128)) {
				const auto [textWidth, textHeight] = interfaces.surface->getTextSize(Surface::font, name);
				interfaces.surface->setTextPosition(ScreenPosition.x, ScreenPosition.y);
