...
public:
	...
	void setDeadFlag(Vector angle) noexcept { 
		const auto localPlayer = interfaces.entityList->getEntity(interfaces.engine->getLocalPlayer());
		*reinterpret_cast<Vector*>((DWORD)localPlayer + 0x31D4 + 0x4) = angle; 
	}
   ...
   constexpr bool setupBones(matrix3x4* out, int maxBones, int boneMask, float currentTime) noexcept
	...
	float GetLowerBodyYawTarget() noexcept
	{
		auto localPlayer = interfaces.entityList->getEntity(interfaces.engine->getLocalPlayer());
		return (float)((uintptr_t)this + localPlayer->lby());
	}
	...
	NETVAR(TakeDamage, "CCSPlayer", "m_takeDamage", char)
