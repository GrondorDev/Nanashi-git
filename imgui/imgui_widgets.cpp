...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	ImColor SliderShine = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .3f);
	ImColor SliderDark = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .5f);
...
bool ImGui::ButtonEx2(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
		//shine and shadow
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 2, bb.Min.y + 1), ImVec2(bb.Max.x - 2, bb.Min.y + 2), SliderShine, style.FrameRounding);
		window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 2, bb.Max.y - 2), bb.Max, SliderDark, style.FrameRounding);
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	ImColor SliderShine2 = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .05f);
	ImColor SliderDark2 = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .3f);
    ...
    {
		ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 10.f)), ImMax(1.0f, IM_FLOOR(square_sz / 10.f)));
		ImVec2 pad2(ImMax(1.0f, IM_FLOOR(square_sz / 9.5f)), ImMax(1.0f, IM_FLOOR(square_sz / 9.5f)));
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, check_col, style.FrameRounding);
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, SliderShine, style.FrameRounding);
		window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 2, check_bb.Min.y + 2), check_bb.Max, SliderDark, style.FrameRounding);
		window->DrawList->AddRectFilled(check_bb.Min + pad2, check_bb.Max - pad2, check_col, style.FrameRounding);
	}
	else {
		ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 10.f)), ImMax(1.0f, IM_FLOOR(square_sz / 10.f)));
		ImVec2 pad2(ImMax(1.0f, IM_FLOOR(square_sz / 9.5f)), ImMax(1.0f, IM_FLOOR(square_sz / 9.5f)));
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, GetColorU32(ImGuiCol_FrameBgHovered), style.FrameRounding);
		window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, SliderShine2, style.FrameRounding);
		window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 2, check_bb.Min.y + 2), check_bb.Max, SliderDark2, style.FrameRounding);
		window->DrawList->AddRectFilled(check_bb.Min + pad2, check_bb.Max - pad2, GetColorU32(ImGuiCol_FrameBgHovered), style.FrameRounding);
	}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	ImColor SliderShine = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .05f);
	ImColor SliderDark = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .1f);
	ImColor SliderShine2 = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .3f);
	ImColor SliderDark2 = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .3f);
    const ImU32 frame_col = GetColorU32(ImGuiCol_FrameBgHovered);
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
		...
		//Textbox Shine
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y), ImVec2(frame_bb.Max.x - 4, frame_bb.Min.y + 2), SliderShine, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//Textbox Shade
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Max.y - 2), ImVec2(frame_bb.Max.x, frame_bb.Max.y), SliderDark, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//Arrow box Shine
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), ImVec2(frame_bb.Max.x - 3, frame_bb.Min.y + 2), SliderShine2, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//Arrow box Horizontal Shine
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), ImVec2(frame_bb.Max.x - arrow_size + 2, frame_bb.Max.y), SliderShine, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//Arrow Box Shadow
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Max.y - 2), ImVec2(frame_bb.Max.x, frame_bb.Max.y), SliderDark2, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//?????
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - 2 - arrow_size, frame_bb.Min.y), ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Max.y), SliderDark, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
		//window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y), ImVec2(frame_bb.Min.x + 6, frame_bb.Max.y), SliderShine, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	const float w = CalcItemWidth();
	ImColor SliderShine = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .3f);
	ImColor SliderDark = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .3f);
	ImColor SliderShine2 = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, .05f);
	ImColor SliderDark2 = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, .1f);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 8.0f, 0.0f));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, id, &frame_bb))
		return false;
	...
	const ImU32 frame_col = GetColorU32(ImGuiCol_FrameBgHovered);
	RenderNavHighlight(frame_bb, id);
	RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 30);
	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x - 2, frame_bb.Min.y + 3), SliderShine2, 30);
	window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Max.y - 2), ImVec2(frame_bb.Max.x - 2, frame_bb.Max.y), SliderDark2, 30);
	...
	if (value_changed)
		MarkItemEdited(id);
	// Render grab      g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
	if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x+2, frame_bb.Min.y + 2), grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrabActive), 30);
		
		//Shine and Shadow
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y + 2), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 8 ), SliderShine, 30);
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 4, frame_bb.Max.y - 2), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y), SliderDark, 30);
	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, -0.3f));

	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Max.x + 5, frame_bb.Min.y), label);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (ButtonEx2("-", ImVec2(button_size, button_size), button_flags))
        {
