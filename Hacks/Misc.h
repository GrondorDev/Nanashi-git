...
namespace Misc
{
	...
	bool fakevote(bool = false) noexcept;
    ...
    {
		if (interfaces.entityList->getEntity(interfaces.engine->getLocalPlayer())->isAlive()){
	...
	constexpr void drawAimbotFov() noexcept
	{
		if (config.misc.drawAimbotFov.enabled && interfaces.engine->isInGame())
		{
			...
			if (config.aimbot[weaponId].silent)
				interfaces.surface->setDrawColor(config.misc.drawAimbotFov.color,255);
			else interfaces.surface->setDrawColor(config.misc.drawAimbotFov.color, 255);
